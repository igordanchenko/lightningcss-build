...
export async function watch({ patterns, inputDir, outputDir, cwd = process.cwd(), silent = false, version, signal }) {
  ...
  // when its build fails (outputPath from the last successful build, or null
  // if there never was one), so unlink bookkeeping stays correct.
  const state = new Map();
  ...
  async function rebuild(entry) {
    try {
      ...
      // Union the deps read before the failure (including the missing file
      // itself) into whatever we were already watching — creating the missing
      // import then triggers the recovery rebuild. Old deps stay in: a partial
      // read may not have reached every file the entry still imports.
      const existing = state.get(entry);
      state.set(entry, {
        outputPath: existing?.outputPath ?? null,
        dependencies: new Set([...(existing?.dependencies ?? []), ...(error.dependencies ?? [])]),
      });
      return null;
    }
  }
  ...
  // The original entry set, fixed at startup. A deleted entry drops out of
  // `state`, but if the same path reappears (git checkout, rename-style editor
  // saves) it re-enters the watch set; files that merely match an entry glob
  // never join it.
    ...
    while (pending.size > 0) {
      ...
      for (const [path, event] of events) {
        if (event === "unlink") {
          ...
          // A deleted file can be an entry and a dependency at once — its
          // importers still rebuild (surfacing the now-broken @import). The
          // deleted entry itself was just dropped from state, so it's not
          // collected here.
          for (const [entry, { dependencies }] of state) {
            if (dependencies.has(path)) affected.add(entry);
          }
        } else {
          // `state` keys are always a subset of `entrySet`, so this also
          // covers changed entries that still have a state record.
          if (entrySet.has(path)) affected.add(path);
          for (const [entry, { dependencies }] of state) {
            if (dependencies.has(path)) affected.add(entry);
          }
        }
  ...
  // The CLI — not this function — turns the returned code into the process
  // exit code.
  return await new Promise((resolveExit) => {
    const shutdown = async () => {
      ...
      closing = true;
      if (timer) clearTimeout(timer);
      // Nothing here may reject: an unsettled promise leaves watch() hanging and
      // the rejection would kill the process with code 1, breaking the
      // documented "SIGINT exits 0" contract.
      try {
        await flushing;
        await watcher.close();
      } catch (e) {
        log.warn(`shutdown error: ${e?.message ?? e}`);
      }
      resolveExit(0);
    };
