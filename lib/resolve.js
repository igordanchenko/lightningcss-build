...
// On Windows, treat backslashes in patterns as path separators (cmd/PowerShell
// tab completion produces them). tinyglobby treats `\` as an escape character,
// so `src\themes\*.css` would otherwise match nothing — and its escaped `*`
// wouldn't even register as a glob. Not tinyglobby's convertPathToPattern:
// that converts *paths* and would escape intentional `{a,b}` braces.
function normalizePattern(pattern) {
  return sep === "\\" ? pattern.replaceAll("\\", "/") : pattern;
}
...
export async function resolveEntries(patterns, cwd = process.cwd()) {
  ...
  for (const rawPattern of patterns) {
    ...
    if (isDynamicPattern(pattern)) {
      ...
      if (matches.length === 0) {
        warnings.push(`warning: no files matched '${rawPattern}'`);
        continue;
      }
      ...
      entries.add(resolve(cwd, rawPattern));
    }
  }
...
export function assertOutputsDoNotOverwriteEntries(entries, inputDir, outputDir) {
  for (const entry of entries) {
    if (resolve(outputDir, relative(inputDir, entry)) === entry) {
      throw usageError(`error: entry '${entry}' would be overwritten by its own output — use a different output dir`);
    }
  }
}
